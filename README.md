...
### 공개 인터페이스 확인
최소한의 공개 인터페이스
- 최소한의 공개 인터페이스를 제공하면 클래스를 가능한 간결하게 할 수 있다.

### 오류 처리를 클래스 속에 설계
협력하기 위한 객체 구성
- 클래스는 다른 클래스에 서비스를 제공하거나 다른 클래스가 제공하는 서비스를 요청하거나 두 가지를 다 한다.
- 클래스를 설계할 때 다른 객체가 어떻게 클래스와 상호작용하는지 반드시 알고 있어야 한다. 

### 확장성을 고려하여 설계
서술형 이름 사용
- 누구나 이름을 읽고서 객체가 나타내는 것이 무엇인지 알 수 있어야 한다. 

이식 불가 코드 추상화
- 이식불가 코드(특정 하드웨어 플랫폼에서만 실행되는 코드)를 사용해야만 하는 시스템을 설계할 경우 클래스로부터 이 코드를 추상화해야 한다.
- 코드를 분리한다는 의미다.
- 예를 들어 래퍼 클래스를 사용할 수도 있다.
- 변경사항이 있을 시 코드에서 유일하게 변경해야 할 곳은 래퍼 클래스가 된다.

가능한 작은 범위 유지
- 이는 추상화와 구현 은닉과 나란히 간다.
- 속성과 행위를 가능하면 로컬화한다는 개념이다.
- 클래서의 유지, 테스트, 확장이 쉬워진다.

### 유지보수를 고려하여 설계
무슨수를 써서라도 공개 인터페이스에 대한 변경은 피해야 한다.

가능한 클래스 간 연결성을 낮추도록 해야 한다.

객체 직렬화와 마샬링
- 객체를 유선을 통해 전송하려면 시스템에서 객체를 분해하고 유선을 통해 전송한 다음 받는 끝에서 재구성해야 하는데 이를 직렬화라한다.
- 유선을 통해 실제로 객체를 보내는 것을 객체 마샬링 이라 한다. 

## Chapter8 프레임워크와 재사용: 인터페이스와 추상 클래스를 이용한 설계
### 코드: 계약이란 무엇인가?
추상클래스
- 하위 클래스가 구현해야만 하는 유일한 메서드는 상위 클래스가 추상이라고 선언한 것이다.
- 이런 추상 메서드는 계약이다.

인터페이스
- 인터페이스는 프레임워크에 대한 계약을 강제하는 강력한 방법이다.

한 곳에 묶기
- Java는 상속, 인터페이스, 조합 이 세가지 방법으로 객체를 구성한다. 
- 엄격한 상속 관계에 있는 클래스는 연관되어야만 한다. 
- 그러나 연관되지 않은 클래스들에 대해 인터페이스를 사용할 수 있다. 
- 이것이 추상클래스를 사용하는 것과 인터페이스를 사용하는 것 간의 중요한 차이점이다.

## Chpater9 객체구성
### 조합 관계
- 조합을 사용하는 이유는 이것이 덜 복잡한 부분을 조합하여 시스템을 구성하기 때문이다. 
- 조합은 부품을 교환할 수 있도록 하는 등 다른 식으로 우리에게 도움을 준다.
- 소프트웨어 개발에서 교환 가능한 부품은 재사용을 의미한다.

### 단계별 구성
- 조합을 사용할 경우 또 하나의 장점은 시스템과 하위 시스템을 독립적으로 구성할 수 있고, 보다 중요하게 독립적으로 테스트하고 유지 관ㄹ리할 수 있다는 점이다.
- 안정적인 시스템을 만들기 위한 방법은 뭐가 있을까?
  - 안정적이고 복잡한 시스템은 보통 각 시스템이 보다 단순한 하위 시스템으로 구성되고 각 하위 시스템이 보다 더 단순한 하위 스스템으로 구성되는 그런 계층 구조의 형태를 취하고 있다. 
  - 안정적이고 복잡한 시스템은 대부분 분해 가능하다.
  - 안정적이고 복잡한 시스템은 거의 항상 상이한 조합으로 정돈된 소수의 하위 스스템으로 구성된다. 
  - 작동하는 안정적인 시스템은 거의 항상 작동하는 단순한 시스템에서 진화한다. 
- 컴포넌트를 사용할 경우 얻는 주요 장점 중 하나는 다른 개발자나 다른 벤더가 구성한 컴포넌트를 사용할 수 있다는 점이다. 

### 조합의 유형
- 일반적으로 조합은 연관과 집합의 두 가지 유형이 있다. 둘 모두 객체간 협력을 나타낸다.
- 집합
  - 집합은 복잡한 객체가 다른 객체들로 구성된다는 의미다.
- 연관
  - 집합이 보통 전체를 보는 관계를 나타내지만, 연관은 전체와 부분을 모두 나타낸다. 
  - 하나의 객체가 서비스를 수행하기 위해 다른 객체를 원하는 경우에 사용된다. 

### 의존 관계 피하기
- 조합을 사용할 때 객체가 서로 지나치게 의존하지 않도록 하는 것이 바람직하다.
- 무슨일이 있더라도 특정 상황을 제외하고는 하나의 도메인에 있는 객체는 다른 도메인에 있는 객체와 혼합하지 않아야 한다. 

### 카디널리티
- 카디널리티를 선택적이든 필수적이든 간에 연관에 참여하는 객체의 숫자로 표현한다. 
- 다중 객체 연관
  - 일대다 관계를 갖는 클래스는 배열로 표현할 수 있다.
- 선택적 연관
  - 연관을 다룰 떄 가장 중요한 이슈 중 하나는 응용 프로그램에서 선택적 연관에 대해 반드시 점검하도록 설계하는 것이다. 
  - 코드에서 연관이 null이 아닌지 반드시 점검해야 한다는 의미다. 
